...
直接繋ぐと認識します。この場合、PC との接続は UART（GP0/GP1）が確実です。

有線モードの USB を PC に繋いだときは、シリアルポートとしても見えます。
UART 変換器が無い場合の戻り口です。`W 0` を送ると無線に戻ります。
...
- **UART 変換器を持たない人は `W 1` を送らないでください。** USB が
  Switch 用（HID）になり、操作口が無くなります。戻し方は 2 つあります。
  1 つは有線モードのまま USB を PC に繋ぎ、見えたシリアルポートから
  `W 0` を送る方法。もう 1 つは電源の入れ直しを 4 回繰り返す方法です
  （各回 60 秒以内に切る）。短い起動が続くと自動で無線に戻ります
