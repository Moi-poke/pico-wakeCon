...
有線モードの USB は HID 単体です。HID＋CDC の複合は PC では動きますが、
Switch が認識しないため採用していません。
...
| `T` / `T <0-2>` | 種類照会・切替（0＝プロコン、1＝Joy-Con L、2＝Joy-Con R。切替後は再起動） |
| `?` | 状態表示（接続状態・保存内容・本体色・モード・種類） |
...
Pro Controller と Joy-Con（L/R）を切り替えられます。無線・有線の
どちらのモードでも使えます。

- 切替えはシリアルから行います（`T 0`＝プロコン、`T 1`＝Joy-Con L、
  `T 2`＝Joy-Con R、`T`＝照会）。切替え後は自動で再起動します
- 種類は Flash に保存されます。種類ごとに MAC アドレスが変わるため、
  ペアリングは種類ごとに行います
- Joy-Con L に十字キーはありません。hat の値が上下左右の押下に
  読み替えられます（`BUTTONS.md` 参照）
- SL/SR は btn の空きビット（`0x4000`/`0x8000`）で送ります。
  Pro Controller では無視されます
