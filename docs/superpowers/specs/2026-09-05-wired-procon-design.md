...
- 実測: CYW43/BT 動作中は Switch 2 ドックが USB に一切触れない (`mnt=0` 不変、Switch 無反応)。電源投入だけ止めた DIAG 版では即 `cfg=1`、`rx80=3/tx81=3` でハンドシェイク開始。記述子・ケーブル・ポート・給電は同一のため、電波・電力が犯人。
- 設計帰結: `W 1` で Classic 切断・待ち受け停止に加え `HCI_POWER_OFF` まで落とし、未列挙なら USB を挿し直したのと同等に戻す (`usb_wired_reconnect`)。`W 0` で電源・待ち受けを戻す。取込 (`C`)・再生 (`B`) の開始時は電波を起こし、終了時に `link_radio_update()` で戻す。BT 内部の初期化順は不変。
- 注意: `K` で消した Pico 側リンク鍵は本物 (再ペア時は Switch 側の登録解除も必要)。`host` / `color` / `saved` が既定表示になるのは BT 起動なしの DIAG 版だけで、通常版では `handle_bt_ready` が読み直すため消えない。
